...
function looper(family) {
  for (var person in family) // family, loop through the given array
    alert(family[person]);//allert every item
}
...
//Write a function named evenFinder that is given nums as it's only argument and removes all values that aren't even
...
function divider(nums, evens, odds) {
  ...
  var o = [];
  var newArray = [e,o];
  for(var v in nums) { //loop through nums array
    if (nums[v] % 2 === 1) {
      o.push(nums[v]);//slice odds and push to o
    }
    else{
      e.push(nums[v]); // slice evens and push to e
    }
  }
  return newArray;
}
...
function reverse(str) {
  var stringy = str.split('').reverse().join('');
  return stringy;
}
...
*/
  //Code Here
  function removeItem(myGroceryList, cutThis) {
    var updated = [];
    for (var i = 0; i < myGroceryList.length; i++){
      if (cutThis === myGroceryList[i]) {
...
/*function removeItem(myGroceryList, cutThis) {
  for (var i = 0; i < myGroceryList.length; i++){
    ...
    myGroceryList.push(cutThis);
  }
  return myGroceryList;
}*/ //obvoiously wrong, but keeping it as a comment for further reference

  function addItem(myGroceryList, addThis){
    var updated = false;
    for (var i=0; i < myGroceryList.length; i++){
      if (addThis === myGroceryList[i]){
        updated = true;
        return myGroceryList;
      }
    }
